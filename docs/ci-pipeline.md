...
- **Runner**: `[self-hosted, linux]` — not pinned to a single runner name.
  The job has no host-specific dependency (no GPU, no per-runner cache) and
  every tool it shells out to comes from this repo's own `flake.nix`, so any
  self-hosted Linux runner in the org can pick it up. Today's eligible
  runners are `ryzen` (32 GB) and `nas` (62 GB, registered after issue #272
  added the render memory caps). Accepted trade-off: the on-demand
  `hetzner-runner` (4 cores / 8 GB) also matches these labels and, if it
  were ever registered, a heavy render landing there would swap and be
  OOM-killed rather than protected by the cap — see the memory-cap design
  decision below. The runner itself provides almost nothing beyond `nix`,
  `git`, and `docker`. Every tool the job shells out to — OpenSCAD (a
...
- **Concurrency**: Groups by `pages-main`, `pages-pr-{N}`, or
  `pages-claws/preview-issue-{id}`. In-progress runs are cancelled when a new
  commit arrives.
...
each on a cold cache — measured on the 32 GB `ryzen` runner) so a
...
#272). The job may run on any self-hosted Linux runner (see the Runner
bullet above), not just `ryzen`. Output is
...
Diffs the PR (or issue-preview) commit to find changed `.scad` files, extracts their top-level
...
- **Main branch**: `aws s3 sync ./site s3://www.bstjohn.net/3d-models/ --delete`
  (excludes `pr-preview/` and `issue-preview/`). Gated on mesh validation, metadata validation, the interference check, parameters-manifest validation, and thumbnail rendering all passing — broken meshes, invalid metadata, overlapping parts, bad manifests, and missing thumbnails never reach production. Not gated on the dependency-graph check: `dependency-graph.md` is a repo doc, not a deployed artifact.
...
- **Issue-preview branch delete**: the same workflow's `delete` trigger
  ...
  `pages-claws/preview-issue-{id}` concurrency group. Guarded by
...
- **Concurrency.** A preview push joins group `pages-claws/preview-issue-<id>`,
...
- **Cleanup.** Claws deletes the branch on Refined or close (writing the
  ...
  manual fallback: it closes any legacy preview PR on the branch (pre-PR-less
  previews) and deletes the branch. The job joins the same
  `pages-claws/preview-issue-<id>` concurrency group, so an in-flight render is
  cancelled before the delete and cannot re-upload afterwards. A
  `workflow_dispatch` with `issue_id` reclaims an orphaned prefix. Because
...
- **Capped OpenSCAD renders**: Every `openscad` invocation in the render
  ...
  **before** its library-detection heuristics, because those exit codes
  ...
  `[self-hosted, linux]` runner (issue clw_01M3ARSS5M37ARAEKERQY1FJ93); the
  28G cap protects a host only if the host has more RAM than the cap.
  `ryzen` (32 GB) and `nas` (62 GB) do, while the on-demand
  `hetzner-runner` (4 cores / 8 GB) would not — a heavy render landing
  there would swap and be OOM-killed, exactly the 2026-07-07 failure mode
  below. This is accepted because that runner is not registered today; a
  shared big-memory label or deriving the cap from the host's RAM is the
  follow-up if it ever bites. Failure detection lives outside this repo
...
runners, an outage of every eligible `[self-hosted, linux]` runner is still

...
- A second laptop-stand variant is requested: "Make a 2nd version of the laptop stand which accepts two laptops," with a concrete spec for the new slot (16mm) and an explicit instruction to keep the original model available, both living in the same directory rather than a new one (#296) — the direct origin of `dual_laptop_stand.scad` alongside the untouched `laptop_stand.scad`.
- Issue #298 opens the repo's largest single project: a Gridfinity-compatible drawer organiser, specified with real caliper/tape measurements (628mm wide at the bottom curving to 665mm at the top, 424mm deep, 69mm tall) and an explicit request to reuse "the modular drawer box system that people use for 3d printing. Grid base and then different size containers all snap into the standard base," constrained by the owner's A1 printer's 250×250mm bed. The owner deliberately scopes the first PR down to "the base for the full drawer and a single 250×250mm container" rather than the whole system — establishing an incremental-delivery pattern the project follows for the next dozen issues. PR #299's only review comments are a functional catch, not a dimensional one: "the bin seems to be a sealed box... the top should be open" — confirmed by opening the model in the 3D viewer, not by any automated check, since a closed extrusion is still a perfectly valid watertight mesh that passes ADMesh/CI validation.
- Issue #304 asks two follow-up questions directly: can bins longer than the 250mm bed be split into printable pieces (and is there a "standard Gridfinity way" of doing this), and can baseplate tiles interlock into a solid base? PR #305 answers both from prior art rather than inventing a scheme from scratch — split-and-glue bins and jigsaw/dovetail-tabbed interlocking baseplates are both established community patterns (linked: Printables/MakerWorld split-bin and interlocking-baseplate models) — and the implementation deliberately omits printed alignment tabs on the bin split, using the baseplate's own pad/socket fit as the alignment jig instead, since a tab at the split plane would be an unsupported overhang below the pad height.
- Issue #309, a photo with the caption "Pieces are completely loose because of jigsaw holes," is a real-world print failure report: the #305 interlock geometry, sited on cell-junction corners for maximum apparent material, actually sat on the *only* rib connecting the tile's perimeter rail to its body, so a notch sized to hold the tab head severed the rail and the whole rail came loose off the bed. PR #310 redesigns the seam around a barbed-tab profile sited at cell centres instead, where the rail is backed by continuous material — a case where a purely dimensional/CAD-verified design (the #305 PR's own writeup claimed ADMesh-clean, watertight geometry) still failed physically, and the fix required print feedback, not more simulation.
- Issue #306 (edge fillers to lock the 40mm-slack baseplate to the drawer) is explicitly opened as **not ready to implement** — "waiting on a test print first" — a deliberate, stated gate on physical validation before more code is written on top of an unconfirmed base. When it does ship (PR #311, following #310's tab redesign), the maintainer's closing comment on #306 documents two departures from the original plan and records them specifically so no one re-actions the stale plan text later: (1) one symmetric filler with notches on both long edges, rather than a handed left/right pair with tabs on one side, trading a fully-locked left edge for a single, non-reversible part; (2) suppressing the front row's unused seam notches was explicitly declined — "they're fine as they are," not carried forward as a follow-up.
- Issue #300 requests the viewer redesign that becomes the tree-browser/multi-pane `index.html`, with one hard constraint stated up front and then repeated as a comment: "The existing links should continue to work after the redesign" — the direct origin of the "never rewrite an incoming hash" rule and the `slugify()`/`PUBLIC_REPO` parity invariants being treated as untouchable during that rewrite.
- Issue #301 traces a documentation gap back to a specific printing experience on the dual laptop stand: preparing to print it revealed that the one genuinely useful piece of guidance (adaptive/variable layer height over the arch's near-horizontal crown, matte filament to hide the rest) had nowhere to live in the repo's data model. The proposal is deliberately scoped to free-text `printing_notes` rather than structured fields (orientation enums, support flags), explicitly citing `ideas/rejected.md`'s prior decline of a structured "print orientation indicator" as the reason to stay prose-only.
- Issue #307 is reported with only a screenshot and the words "Bad layout" — another instance of the terse, image-only bug report pattern, here diagnosed by the implementer as the gallery's fixed-height non-scrolling app shell leaving no room for a phone-width pane's chrome.
- Issue #313 asks a pointed, self-answering question rather than asserting a bug outright: "the full assembly including the spacers at the side makes the bottom look as wide as the top. Is it?" — given the 628→665mm taper the owner specified back in #298, a preview whose floor and rim read the same width is self-evidently wrong, and the implementer's diagnosis confirms two independent geometry defects (a flare that tracked the drawer's own slope instead of closing the gap to the wall, and container walls silently built at double their documented thickness).
- Issue #315 supersedes the #298-era floor arithmetic with a physically confirmed correction: "Three 5x5 base grids actually fit the width of the drawer at the bottom" — i.e. 3×210mm = 630mm fits where the original conservative 628mm measurement implied only a 14-column/588mm grid with leftover slack needing filler strips (#306/#311). The owner also specifies the follow-on container layout precisely (one 3×10 left bin, one 3×12 back bin, three 4×7 front bins), which the implementation adopts verbatim. This is the second time in this project a maintainer-supplied physical remeasurement has overridden an earlier conservative estimate baked into the geometry (the first being the #309 interlock-position failure) — a recurring theme that the drawer's nominal dimensions are treated as provisional until a real print or a closer look confirms them.
- Issue #317 asks for the full-drawer assembly's five containers to each be individually downloadable — until this point `drawer_assembly.scad` was a preview-only merged mesh with no printable path to its own contents, and the fix (`container_part()`, bed-splitting each container the same way `bin_part()` already split oversized bins) extends the project's bed-splitting convention to the assembly's own parts rather than introducing a new mechanism.

### 2026-07-26

- Issue #319: the owner states the printer's real constraint plainly — "Printer max is 250x250mm" — and asks that each oversized container be split "according to the 5cells max underneath it," with the resulting pieces "butted against each other & superglued." This is a physical-constraint-driven request (not a code cleanliness one) and is implemented by extracting `container_slice()` out of `container_part()` so pieces can take an explicit cell range instead of an even split.
- Issue #322, opened the very next day, refines #319's split placement: "Try not to align the splits in the drawer containers with the splits in the grid below. Ha[v]ing a solid container piece straddle a grid join will make it stronger, also having t[w]o container pieces meet in the middle of a single grid piece should help them sit flush too if not superglued." A follow-up human comment carves out one explicit exception: "The left container can stay as two 5sq pieces, that's preferable than introducing another cut" — i.e. the owner accepts an on-seam split when the alternative is a third cut, showing the seam-avoidance preference is about piece count/quality tradeoffs, not an absolute rule. This is the origin of the `container_slice()` piece files' hand-picked, seam-offset `c0`/`c1` boundaries.
- Issue #324 requests a full relayout of the drawer's container grid, given as a terse enumerated spec plus an attached sketch: left stays 3×10; back row becomes three 4×6 containers with the outer one flared; front row becomes 8×4, "3x3", and 1×3 + 1×1 (both flared) — deliberately superseding the earlier 3×10/12×3/four-4×7 layout from #298/#315. The implementer resolved a genuine ambiguity between the issue text's "3x3" and the sketch's clear 4-row depth by choosing 3×4 (the only reading that tiles the 15×10 grid exactly with no gap) and documented that judgment call for the maintainer to correct if wrong — as of this writing it has not been corrected, so 3×4 stands. This supersedes the #319/#322 split-boundary work for every piece file it touches (the old back/front containers and their pieces are deleted and replaced under new names, never reusing old slugs so stale QR/deep-links fail cleanly instead of silently resolving to a different shape); only the untouched left container keeps its #319/#322 geometry.
- Issue #326 is a real-print correction filed immediately after the first physical print of the new layout, with photos: "The overall height is wrong. The 3x1 piece sitting into the base is 56mm high in total but the drawer height is 69mm and the containers should be using all of it. Also the flared side to the right does not meet the edge." A follow-up comment supplies the corrected measurement that had been an estimate until now — "Initially I said the top of the drawer was 665 wide but it's more like 669/670" — and confirms the geometric assumption underpinning the straight-taper flare model is still safe: "the curved walls of the drawer curve away rapidly at the bottom so no danger of a straight flare hitting it." Both fixes (container height quantized to Gridfinity `z_units` → full 69mm drawer height in mm; `drawer_top_w` 665 → 670) are dimensional corrections arriving from a physical print, continuing the project's established pattern (see #309, #313, #315) of trusting a real print or remeasurement over an earlier modeled estimate. Bins stay on `z_units`/56mm — they are genuinely Gridfinity-stackable and this correction only applies to containers, which are not.

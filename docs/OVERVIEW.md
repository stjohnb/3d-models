...
|---|---|---|
| [PRODUCT.md](PRODUCT.md) | Deciding what the product must do and why | **Entry point** |
| [OVERVIEW.md](OVERVIEW.md) (this doc) | Starting technical work in this repo | **Entry point** |
...
├── heating-controller-box/  # Two-part wall enclosure for the CYD heating controller, mains bay partitioned off
...
├── resistor-storage-box/  # Open-top bag-file box for resistor kit bags, end-wall thumb scoops, push-on lid
...
│   ├── request-issue-preview.sh  # Pushes candidate geometry to claws/preview-issue-<id> so CI renders it to issue-preview/<id>/<sha8>/ without a PR; Claws retires the branch on Refined/close; `--cleanup` is the manual fallback; see playbooks/preview_a_proposed_model.md
│   ├── preview_summary.py      # CI: writes an issue preview's preview-summary.json (thumbnails, sizes, triangles, validation, interference, markdown)
...
│   ├── image_metadata.py       # Finds/strips embedded EXIF/XMP/IPTC/comment metadata from tracked images (issue #536)
│   └── test_image_metadata.py  # Pins every tracked image clean of metadata
...
├── claws.json            # Opts this repo into Claws automation (`{"enabled": true}`); per-job disables live here in `disabledJobs` (unioned with the automation host's config); `issuePreviewSummaryUrl` tells Claws where PR-less preview summaries deploy — see claws-automation.md
├── .agents/
...
│   └── preview_a_proposed_model.md  # How to request a PR-less CI render preview during issue planning
├── .github/
...
│       └── pr-preview-cleanup.yml  # CI: delete pr-preview/pr-{N}/ on PR close and issue-preview/{id}/ on preview-branch delete
└── docs/
    ...
    ├── PRODUCT.md              # Product intent and requirements index
    ├── product/                # Focused product-requirement areas (indexed by PRODUCT.md)
...
Python script tests are auto-discovered by `python3 -m pytest scripts/`.
...
| `heating-controller-box/` | Two-part printed wall enclosure for the ESP32-2432S028R heating and hot-water controller: partitioned base with relay/PSU/WAGO mounts and a stylus sleeve, faceplate with framed display, Hand-Off-Auto toggles and indicator lamps; heat-set inserts throughout |
...
- **`playbooks/scan_a_capture.md`** — how to run `scan_pipeline.py` over a
  ...
  platter.
- **`playbooks/preview_a_proposed_model.md`** — how to request a PR-less
  issue preview so proposed geometry renders on CI, and stays current with
  the plan, before it becomes issue implementation work.

## AI Agent Configuration

`AGENTS.md` is the canonical and only root agent guide; Claws inlines it into
every run. `.agents/*.md` role documents are layered on top of it. `AGENTS.md`'s
"Automation host policy" section is a verbatim copy of Claws' canonical
`HOST_POLICY_MARKDOWN` block, checked daily by Claws' `host-policy-scanner`; keep
the three canonical bullets word-for-word and add repo-specific host rules as extra
bullets below them. Claws — an autonomous agent service — manages this repo's
issues, PRs, and docs; see [claws-automation.md](claws-automation.md). Product intent
lives in [PRODUCT.md](PRODUCT.md); durable operator/automation gotchas that
belong to no feature doc live in [agent-notes.md](agent-notes.md).
...
`build.yml` runs on push to `main` and on PRs, on any `[self-hosted, linux]`
runner (render memory caps are a hard ceiling every eligible runner must
exceed, see [ci-pipeline.md](ci-pipeline.md)). All tooling comes from this
repo's own `flake.nix` devShell, not the runner host. The pipeline renders
every `.scad` to STL, validates meshes and
...
| Item | Location | Notes |
|------|----------|-------|
| CI runner | `[self-hosted, linux]` in `build.yml` | Runner provides only `nix`/`git`/`docker`; OpenSCAD, ImageMagick, ADMesh, qrencode, zip, Python 3, Node.js, and the AWS CLI all come from this repo's `flake.nix` `default` devShell, entered via `.github/actions/setup-nix` + the job-level `defaults.run.shell`. `build.yml`'s `build` job runs on any self-hosted Linux runner with more RAM than `RENDER_MEM_MAX` (currently `ryzen` and `nas`) — the cap is not calibrated per host, and an under-provisioned runner matching the labels would be OOM-killed rather than protected |
...
| Issue preview path | `s3://…/issue-preview/{id}/{SHA}/` | PR-less issue-planning previews from `claws/preview-issue-{id}` pushes, with `preview-summary.json`; deleted recursively by `pr-preview-cleanup.yml` when the branch is deleted |

...
Product requirements: [product/printable-models.md](product/printable-models.md)
...
Two-part, fully printed wall enclosure for the ESP32-2432S028R (CYD) heating
and hot-water controller that replaces a Danfoss FP720 programmer. The build
itself (schematic, parts, wiring order) lives in the controller's build notes,
outside this repository. The base tray screws
to the wall in place of the FP720; the faceplate lid frames the display and
carries the two Hand-Off-Auto toggles and the two 230 V indicator lamps.

| File | Role |
|------|------|
| `_heating_controller_box.scad` | Shared library — all parameters, asserts and modules (`rbox`, `insert_hole`, `flat_teardrop`, `toggle_cutout`, `cradle`, `partition`, `stylus_sleeve`, `heating_controller_base`, `lid_frame`, `heating_controller_lid`); no top-level geometry |
| `heating_controller_base.scad` | Renderable — wall-mounted tray with partition rib, relay bosses, PSU/WAGO cradles, corner posts, wall screws, cable entries, vents and stylus sleeve |
| `heating_controller_lid.scad` | Renderable — faceplate with framed display window, CYD bosses, keyed toggle holes, lamp holes, locating lip and countersunk lid screws |
| `heating_controller_base.parameters.json` | Customizer manifest for the base (cavity size, inserts, relay boss height and X position, PSU cradle, cable entries, vents, stylus sleeve) |
| `heating_controller_lid.parameters.json` | Customizer manifest for the lid (cavity size, inserts, window size/offset, CYD glass height, toggle/lamp cut-outs and positions, lid thickness) |
| `meta.json` | Project metadata, hardware BOM and printing notes |
| `dependency-graph.md` | Auto-generated `include` dependency graph |

**Layout frame and bays**: every position is in front-view coordinates — X
right and Y up as seen from the room, origin at the box centre. The base floor
(z=0) is the face against the wall and uses those coordinates directly. The
lid is modelled with its outer face at z=0 and printed face-down as
`mirror([1,0,0]) lid_frame()`, which keeps +Y as the top of the fitted
faceplate. The cavity is 164 x 128 x 40 mm. An L-shaped partition rib splits
it into a top-left low-voltage bay for the CYD and a mains bay made of the
bottom strip (PSU and WAGO block) and the right-hand column (toggles and
lamps); a 20 mm notch in the rib passes the CYD pigtail and the PSU 5 V
leads. The relay board sits right of the vertical rib, below the lower
toggle. Its low-voltage header faces up, at least `relay_header_clear`
(10 mm) from the toggle's 230 V terminals, and its screw terminals face a
`relay_terminal_clear` (12 mm) wiring channel along the bottom wall. Asserts
keep both gaps and stop the relay, toggles, rib and cradles from colliding.
The lid locates with a lip and screws down — no snap fit on a mains box.

**Fixings**: every screw goes into an M3 heat-set insert (12 in total), cut by
the single `insert_hole(depth)` module: a `insert_hole_d` (4.0) pocket at
least 6 mm deep with a 0.5 mm entry chamfer, in a boss or post at least 7 mm
across. Setting `insert_hole_d` to 2.6 turns every pocket into a self-tapping
pilot. The relay board screws straight onto four 8 mm printed bosses
(`relay_boss_h` 8–10 mm, clearing the solder side), so the printed plastic
insulates it instead of nylon standoffs. The four corner posts are 8 mm across
(a 7 mm post leaves only about 1 mm of wall round the chamfered mouth) and
their pockets are `post_insert_depth` (8 mm) deep, so an M3 x 10 countersunk
lid screw through the 3 mm lid does not bottom out. The four 7.5 mm CYD
bosses on the lid underside set the glass `cyd_boss_gap` behind the lid; their
pocket depth is clamped to leave `lid_skin` (1.2 mm) on the display face.

**Toggle cut-out**: a 12 mm circle with a 1 mm-deep key tab of lid material
at 12 o'clock, matching the XT-13A legend plate, so the switch travel is
vertical and the ON legend reads at the top. `toggle_key_depth = 0` removes
the tab.

**Stylus sleeve**: a solid 12.4 mm block on the outside of the +X wall,
standing on the wall face, with its bores running along Y. The 5.6 mm shaft
bore runs the full length and is open at the bottom so a stuck stylus can be
pushed out; an 8.4 mm counterbore 8 mm deep takes the 12 mm cap and leaves
4 mm of it proud to grip. Both bores use a flat-topped teardrop so they print
without supports. The sleeve is added after the base's cuts, so the +X wall
stays solid and the sleeve never opens into the mains bay. It widens the base
to about 180 mm.

**Measured vs assumed**: measured (calipered 23 September 2026 and later
issue comments) — the CYD and relay hole pitches, the toggle body, hole and
key depth, the PSU and WAGO bodies, and the stylus. Assumed and exposed or
marked VERIFY — `cyd_display_h` and whether the glass is centred on the
holes (`window_off_x`/`window_off_y`), the toggle key width, the legend-plate
and lamp-bezel footprints (clearance asserts only), the lamp hole, the PSU
stripboard cradle (62 x 30), the PCB thicknesses and the stylus bore
clearances.
...
An open-top box, sized to the issue's 10 x 7 x 7 cm envelope, that files
resistor-kit bags on edge like index cards rather than dumping them loose.
A separate push-on lid closes it once the bags are loaded.

| File | Role |
|------|------|
| `_resistor_storage_box.scad` | Shared library — `$fn = 64`, all parameters, modules `rbox()` (rounded-rectangle prism), `scoop_cutter()`, `resistor_box()`, `resistor_box_lid()`; no top-level geometry |
| `resistor_box.scad` | Renderable — open-top box with a thumb scoop in each end wall; prints floor-down, no supports |
| `resistor_box_lid.scad` | Renderable — flat lid with an inner locating lip; modelled top-face-down so it also prints flat, no supports |
| `resistor_box.parameters.json` | Customizer manifest — `outer_l`, `outer_w`, `outer_h`, `wall`, `floor_t`, `corner_r`, `scoops`, `scoop_w`, `scoop_depth` |
| `resistor_box_lid.parameters.json` | Customizer manifest — `outer_l`, `outer_w`, `wall`, `corner_r` (shared with the box), `lid_t`, `lip_h`, `lip_t`, `lid_clearance` |
| `meta.json` | Project metadata (description, tags: electronics/organizer/storage/workshop, difficulty: beginner, version 0.1.0; no `mating_pairs` — the lid's as-rendered pose is its print pose, not its fitted pose) |
| `dependency-graph.md` | Auto-generated `include` dependency graph |

Bags stand on their 100 mm edge along `outer_l` and file across the ~67 mm
cavity width, the way index cards stand in a card box; a 25-value 1/4 W kit at
roughly 2.6 mm per bag fills that width with no divider ribs. Ribs were
rejected because at that pitch they would consume the cavity width they are
meant to organize; the plain cavity is the "better" storage the issue asked
for.

Each end wall (the `outer_w` x `outer_h` faces) carries a U-shaped thumb
scoop — a horizontal round-bottomed cutout `scoop_w` wide reaching
`scoop_depth` below the rim — so a single bag's end can be pinched and lifted
out without disturbing its neighbours, and so a thumb can push the fitted lid
up from below. Set `scoops` to `false` for a plain wall.

The lid is a `lid_t` plate carrying an inner locating lip that drops `lip_h`
into the cavity with `lid_clearance` per side; an inner lip keeps the lid's
footprint identical to the box's rather than growing it with an outer skirt.
`outer_l`, `outer_w`, `wall` and `corner_r` are shared between the two
manifests — change them on both the box and the lid, or the lid will not fit.

...
**Depth: Deep dive.** Read this only when editing this blog draft. For repo
architecture read [OVERVIEW.md](OVERVIEW.md) instead.

**Voice.** This is a personal blog talking about a hobby, not marketing copy
or a technical reference — keep a relaxed, first-person tone. Avoid
LLM-generic patterns: short punchy sentences and constant emphasis on how
great everything is. Keep friction and false starts in the narrative (a tight
first fit, a lost part) rather than smoothing the story into a success
montage. **Why:** the owner rejected an earlier draft for reading as
obviously LLM-generated in tone (issue #243).
...
The viewer is a single `index.html` with no build step — no React, no bundler, no npm. It's plain ES modules, with Three.js loaded through an import map from a copy the build vendors in same-origin rather than pulling from a CDN. There's a tree of every project down one side, and clicking a model loads it into one of up to three panes on the other, each with orbit controls and a row of filament-colour swatches so you can see roughly what it'll look like printed. Only the panes you actually open spin up a WebGL context, so the page doesn't get any heavier as the collection grows. For something I mostly look at to check a model came out right, keeping it to one file has been very little hassle to maintain. It's all live at [bstjohn.net/3d-models](https://www.bstjohn.net/3d-models/) if you want to spin any of these around yourself.

// =====================================
// Elegant Arch Laptop Stand
// A swept arch into which the closed laptop slides edge-down. The slot is a
// flat-floored channel cut straight DOWN from the crest: the laptop's bottom
// edge seats on a level floor (an XY face) between two vertical side walls,
// rather than a groove that follows the curved arch band.
// Modelled after elegant-arch-macbookpro-stand.stl (arch curves sampled
// from that mesh) but fully parametric — the slot width is sized to the
// laptop thickness.
//
// Coordinate system: Z up, base on Z=0. X = width (foot-to-foot),
// Y = depth (front-to-back), slot runs along X centred on Y=0.
...
// === Parameters (mm) ===
slot_gap     = 18;    // laptop slot width — set to laptop thickness + clearance
groove_depth = 70;    // how deep the laptop sinks into the arch band
stand_width  = 240;   // overall width, foot-to-foot (X)
stand_depth  = 150;   // overall depth at the base, front-to-back (Y)
...
// === Central laptop slot — flat-floored channel cut into the arch band ===
// A rectangular channel, slot_gap wide in Y and slot_length long in X, cut
// straight DOWN from above to a flat horizontal floor at floor_z. All material
// stays within the arch band, so the slot's vertical side walls (XZ faces at
// Y = +/- slot_gap/2) follow the band over the crest and the laptop's bottom
// edge seats where the band drops below floor_z near the legs — there is no
// continuous floor bridging the two sides of the arch opening.
module slot_cut() {
    floor_z = stand_height - groove_depth;   // height of the flat seating floor
    translate([-slot_length/2, -slot_gap/2, floor_z])
        cube([slot_length, slot_gap, stand_height + 2 - floor_z]);
}

...
full render pipeline (the `build` job) run on PR close and re-upload the
very preview this workflow just deleted, since "Deploy PR preview" is
gated only on `github.event_name == 'pull_request'`.
...
class DeleteSafetyTests(unittest.TestCase):
    ...
    def test_delete_target_is_scoped_to_one_pr(self):
        text = read(CLEANUP_YML)
        self.assertEqual(text.count("aws s3 rm"), 2)
        self.assertNotIn('pr-preview/" --recursive', text)
        self.assertNotIn('pr-preview" --recursive', text)
        self.assertNotIn('issue-preview/" --recursive', text)
        self.assertIn('target="${BUCKET_PREFIX}/pr-${pr}/"', text)
        self.assertIn(
            'target="s3://www.bstjohn.net/3d-models/issue-preview/${ISSUE_ID}/"',
            text,
        )

    def test_issue_id_is_validated_before_delete(self):
        text = read(CLEANUP_YML)
        self.assertIn("^([0-9]+|clw_[A-Za-z0-9]+)$", text)
        self.assertIn("Refusing to delete: '$ISSUE_ID' is not an issue id", text)

    def test_values_not_interpolated_into_run_commands(self):
        text = read(CLEANUP_YML)
        run_marker = "        run: |\n"
        starts = [m.end() for m in re.finditer(re.escape(run_marker), text)]
        # Validate workflow_dispatch inputs, Delete PR preview prefixes,
        # Delete issue preview prefix.
        self.assertEqual(len(starts), 3)
        for start in starts:
            end = text.find("\n      - ", start)
            run_block = text[start:] if end == -1 else text[start:end]
            self.assertNotIn("${{", run_block)


class IssuePreviewCleanupTests(unittest.TestCase):
    def test_triggers_on_branch_delete(self):
        text = read(CLEANUP_YML)
        self.assertRegex(text, r"(?m)^  delete:\s*$")
        self.assertIn("github.event.ref_type == 'branch'", text)
        self.assertIn(
            "startsWith(github.event.ref, 'claws/preview-issue-')", text
        )

    def test_dispatch_accepts_issue_id(self):
        text = read(CLEANUP_YML)
        self.assertRegex(text, r"(?m)^      issue_id:\s*$")

    def test_concurrency_group_matches_build_issue_group(self):
        """build.yml's issue-preview group is 'pages-' + the branch name, so
        a branch delete or issue_id dispatch cancels an in-flight render
        before the prefix is deleted.
        """
        build_text = read(BUILD_YML)
        self.assertIn(
            "(startsWith(github.ref, 'refs/heads/claws/preview-issue-') && github.ref_name)",
            build_text,
        )
        cleanup = read(CLEANUP_YML)
        self.assertIn("format('pages-{0}', github.event.ref)", cleanup)
        self.assertIn(
            "format('pages-claws/preview-issue-{0}', inputs.issue_id)", cleanup
        )
